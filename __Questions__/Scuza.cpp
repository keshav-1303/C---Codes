...
void solve(){
    ...
    vector<ll> v;    inputVector(v, n);     // v : stair length

    vector<ll> len;     inputVector(len, q);     // len : length of Timur's legs
    ...
    for(ll i=0 ; i<q ; i++){
        ...
        auto upper = upper_bound(v.begin(), v.end(), len[i]);

        ll sum = accumulate(v.begin(), upper, 0);

        // while(len[i] >= v[c]  &&  i < q  &&  c < n){
        //     sum += v[c];
        //     c++;
        //     // debug(sum);
        // }
